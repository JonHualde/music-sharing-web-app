    ...
    <section class="container mx-auto mt-6">
      <div class="bg-white rounded border border-gray-200 relative flex flex-col">
        <div class="px-6 pt-6 pb-5 font-bold border-b border-gray-200">
          <!-- Comment Count -->
          <span class="card-title">{{ $tc("songPage.comment_count", song.comment_count, { count: song.comment_count }) }}</span>
        ...
        <div class="px-6 pt-6 pb-5 font-bold border-b border-gray-200">
          <ul class="container mx-auto">
            <li v-if="comments.length === 0">
              <div class="mb-5">
                <div class="font-bold">No comment yet.</div>
              </div>
            </li>
            <li v-else v-for="comment in sortedComments" :key="comment.docID" class="p-6 bg-gray-50 border border-gray-200">
              <!-- Comment Author -->
              <div class="mb-5">
...
export default {
  ...
  methods: {
    ...
    async addComment(values, context) {
      ...
      await songsCollection.doc(this.$route.params.id).update({ comment_count: this.song.comment_count });
